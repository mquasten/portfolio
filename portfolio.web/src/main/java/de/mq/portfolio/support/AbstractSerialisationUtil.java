...
abstract class AbstractSerialisationUtil implements SerialisationUtil {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see de.mq.portfolio.support.SerialisationUtil#toMap(java.lang.Object,
	...
	public final String serialize(final Map<String, Object> values) {
		...
		return ((ExceptionTranslationBuilder<String, ByteArrayOutputStream>) builder()).withResource(() -> new ByteArrayOutputStream()).withTranslation(ResourceAccessException.class, Arrays.asList(IOException.class)).withStatement((os) -> {
	 ...
	 * 
	 * @see
	 * de.mq.portfolio.support.SerialisationUtil#deSerialize(java.lang.String)
	 */
	@Override
	@SuppressWarnings("unchecked")
	...
	public final <T> void toBean(final Map<String, Object> map, final T target, final Collection<String> mapping) {
		...
		map.keySet().stream().filter(key -> notContains(blackList, key)).forEach(key -> {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see de.mq.portfolio.support.SerialisationUtil#execute(java.lang.Object,
	...
	private Collection<String> arguments(final Method m) {
		...
		Arrays.asList(m.getParameterAnnotations()).forEach(array -> Arrays.asList(array).stream().filter(a -> a.annotationType().equals(Parameter.class)).map(a -> ((Parameter) a).value()).filter(name -> notContains(results, name)).forEach(name -> results.add(name)));
		return Collections.unmodifiableCollection(results);
	}

	private boolean notContains(final Collection<String> results, String name) {
		return !results.contains(name);
	}

	/*
	 * (non-Javadoc)
	 * @see de.mq.portfolio.support.SerialisationUtil#getAndIncVersion(java.lang.Object)
	 */
	@Override
