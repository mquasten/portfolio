...
public class ShareGatewayParameterServiceTest {
	...
	public final void dependencies() throws BeanInstantiationException, NoSuchMethodException, SecurityException {
		...
		Assert.assertEquals(gatewayParameterRepository, DataAccessUtils.requiredSingleResult(Arrays.asList(ShareGatewayParameterServiceImpl.class.getDeclaredFields()).stream().filter(field -> dependencies.containsKey(field.getType())).map(field -> ReflectionTestUtils.getField(service, field.getName())).collect(Collectors.toList())));
	}
}
