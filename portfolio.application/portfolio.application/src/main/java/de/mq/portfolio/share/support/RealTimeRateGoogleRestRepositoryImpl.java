...
abstract class RealTimeRateGoogleRestRepositoryImpl implements RealTimeRateRepository {
	...
	public Collection<TimeCourse> rates(final GatewayParameterAggregation<Collection<Share>> gatewayParameterAggregation) {
		...
		return IntStream.range(0, parameters.size()).filter(i -> !((List<Share>) gatewayParameterAggregation.domain()).get(i).isIndex()).mapToObj(i -> rates(configurableConversionService, gatewayParameter.urlTemplate(), parameters.get(i), ((List<Share>) gatewayParameterAggregation.domain()).get(i)))
	...
	private Data newData(final Date date, final double value) {
		...
		Arrays.asList(DataImpl.class.getDeclaredFields()).stream().filter(field -> field.getType().equals(DateFormat.class)).forEach(field -> {
			field.setAccessible(true);
			ReflectionUtils.setField(field, data, df);
		});
		return data;
	}
